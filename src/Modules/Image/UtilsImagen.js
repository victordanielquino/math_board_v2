...
const u_getImageData = (context, imagen) => {
	return context.getImageData(
		imagen.x_ini,
		imagen.y_ini,
		imagen.x_fin - imagen.x_ini,
		imagen.y_fin - imagen.y_ini
	);
};
	...
	if (objImagen.visible) {
		...
		imagen.addEventListener('load', () => {
			context.drawImage(
				...
				objImagen.x_ini,
				objImagen.y_ini,
				objImagen.x_fin - objImagen.x_ini,
				objImagen.y_fin - objImagen.y_ini
			);
		});
	}
};
const u_imageDrawImageSrc = (context, objImagen) => {
	...
	newPromise = new Promise((aceptado, rechazado) => {
		imagen.addEventListener('load', () => {
			imagen.src === ''
				? rechazado(new Error('drawImageGoogle: no se pudo modificar!!!'))
				: '';
			aceptado(
		...
		if (image.rotateDeg !== 0) {
			...
			aceptado(
				context.drawImage(
					...
					image.x_ini,
					image.y_ini,
					image.x_fin - image.x_ini,
					image.y_fin - image.y_ini
				)
			);
		}
	});
...
const u_imagenBuscaPtoClickParaRedimencionar = (image, x, y) => {
	...
	for (let i = 0; i < image.vertex.length; i++) {
		if (
			image.vertex[i].x - 5 < x &&
			x < image.vertex[i].x + 5 &&
			image.vertex[i].y - 5 < y &&
			y < image.vertex[i].y + 5
		) {
...
const u_imagenUpdateZise = (image, mouse) => {
	...
	switch (mouse.imagen_pto) {
		case 0:
			...
			let recMouse = rectaQuePasaPorDosPtos(
				{ x: mouse.pos.x, y: mouse.pos.y },
				{ x: image.h, y: image.k }
			);
			let cirRadio = circunferenciaConCentroRadio(
				{ h: image.h, k: image.k },
				image.radio
			);
			...
			if (d1 < d2) {
				image.vertex[0].x = resp1.x1;
				image.vertex[0].y = resp1.y1;
				image.vertex[2].x = resp1.x2;
				image.vertex[2].y = resp1.y2;
			} else {
				image.vertex[0].x = resp1.x2;
				image.vertex[0].y = resp1.y2;
				image.vertex[2].x = resp1.x1;
				image.vertex[2].y = resp1.y1;
			}
			const p3 = { x: image.vertex[3].x, y: image.vertex[3].y };
			let recAngulo = anguloEntreDosRectasCaso1(recMouse, 180 - image.angulo, {
				x: image.h,
				y: image.k,
			});
			...
			if (d1 < d2) {
				image.vertex[3].x = resp2.x1;
				image.vertex[3].y = resp2.y1;
				image.vertex[1].x = resp2.x2;
				image.vertex[1].y = resp2.y2;
			} else {
				image.vertex[3].x = resp2.x2;
				image.vertex[3].y = resp2.y2;
				image.vertex[1].x = resp2.x1;
				image.vertex[1].y = resp2.y1;
			}
			// pto: 4
			...
			// ROTATE IMAGE:
			let recHorizontal = rectaQuePasaPorDosPtos(
				{ x: image.h - 1, y: image.k },
				{ x: image.h, y: image.k }
			);
		...
		case 4:
			...
			recParalalela = u_rectaParalela(recCentro, {
				x: mouse.pos.x,
				y: mouse.pos.y,
			});
			...
			image.k = ptoMedio.y;
			// angulo:
			rec1 = rectaQuePasaPorDosPtos(image.vertex[0], {
				x: image.h,
				y: image.k,
			});
			rec2 = rectaQuePasaPorDosPtos(image.vertex[3], {
				x: image.h,
				y: image.k,
			});
			...
			// radio:
			image.radio = u_distanciaEntreDosPtos(image.vertex[0], {
				x: image.h,
				y: image.k,
			});
			// rotateDegPrev:
		...
		case 6:
			...
			image.k = ptoMedio.y;
			// angulo:
			let r1 = rectaQuePasaPorDosPtos(image.vertex[0], {
				x: image.h,
				y: image.k,
			});
			let r2 = rectaQuePasaPorDosPtos(image.vertex[3], {
				x: image.h,
				y: image.k,
			});

...
import {u_textGraficaFontEdit, u_textGrafica, u_textGraficaH2, u_textGraficaH} from "./UtilsText";
        ...
        if (stateText.active){
            ...
            try {
                ...
                state.historia.forEach((elm, index) => {
                    ...
                    switch (elm.type){
                        case 'pencil':
                            (elm.visible && elm.canvas === state.canvas)
                                ? u_pencilDraw(context, elm) : '';
                            break;
                        case 'text':
                            if(elm.canvas === state.canvas) {
                                if (stateText.fontFocus) {
                                    let bool = false;
                                    (index === state.historia.length -1) ? bool = true: '';
                                    u_textGraficaFontEdit(context, elm, bool);
                                }
                                else {
            ...
            state.historia.forEach((elm, index) => {
                ...
                switch (elm.type){
                    case 'pencil':
                        ...
                        break;
                    case 'text':
                        if(elm.visible && elm.canvas === state.canvas) {
                            if (stateText.fontFocus) {
                                let bool = false;
                                (index === state.historia.length -1) ? bool = true: '';
                                u_textGraficaFontEdit(context, elm, bool);
